...
var MAX_REQUEST_DURATION = 59000; // ms
...
function createDBHandler(key, url, callback) {
    ...
    var stopTimeout = function () {
        clearTimeout(tid);
        db.unsub(key, handleStatus);
    };

    var startTimeout = function () {
        db.sub(key, handleStatus);
    ...
    var handler = function (err, val) {
        if (err) {
            ...
            db.set(key, { pending: true, time: Date.now() });
            getSession(key, url);
            startTimeout();
            return;
        }
        ...
        if (val.session) {
            ...
            if (Date.now() - (val.time || 0) > FIVE_MINUTES) {
                console.error('giving up on this one...');
                // db.del(url);
...
module.exports = {
    viewifyURL: function (url, callback) {
        var key = md5(url);
        db.get(key, createDBHandler(key, url, callback));
    },
